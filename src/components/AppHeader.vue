...
watch(() => isInfoModalOpen.value, (value) => {
  if (!value) {
    sessionStorage.setItem("isInfoModalOpen", 0);
  }
});

onMounted(() => {
  const isInfoModalOpenStored = sessionStorage.getItem("isInfoModalOpen") ?? 1;

  isInfoModalOpen.value = parseInt(isInfoModalOpenStored) ? true : false;
});
...
<template>
  ...
  <diary-settings-modal v-model="isDiarySettingsModalOpen"
    @created="(diaryId) => emit('diary-created', diaryId)"></diary-settings-modal>

  <health-record-setings-modal v-model="isHealthRecordSettingsModalOpen"
    @created="(diaryId) => emit('health-record-created', diaryId)"></health-record-setings-modal>

  <v-container fluid class="header-bg" min-width="400">
    <v-row justify="center" class="mb-2 d-flex flex-wrap">
      <v-col cols="2">
        <v-btn color="primary" size="45" rounded="xl" class="mr-2 mb-2 min-w" @click="isInfoModalOpen = true">
          <svg xmlns="http://www.w3.org/2000/svg" height="25" width="12" viewBox="0 0 192 512">
            <path :fill="backgroundColor"
              d="M48 80a48 48 0 1 1 96 0A48 48 0 1 1 48 80zM0 224c0-17.7 14.3-32 32-32l64 0c17.7 0 32 14.3 32 32l0 224 32 0c17.7 0 32 14.3 32 32s-14.3 32-32 32L32 512c-17.7 0-32-14.3-32-32s14.3-32 32-32l32 0 0-192-32 0c-17.7 0-32-14.3-32-32z" />
          </svg>
        </v-btn>
      </v-col>
      <v-col class="d-flex justify-center text-h4 text-center">Personal diary </v-col>
      <v-col cols="2"></v-col>
    </v-row>
      ...
      <v-col class="d-flex justify-center flex-wrap">
        <v-btn class="mx-2 mb-3 mb-md-0" @click="isDiarySettingsModalOpen = true">
          <template v-slot:prepend>
            <svg xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 512 512" class="my-2">
